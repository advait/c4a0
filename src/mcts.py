...
AsyncEvaluatePos = Callable[[Pos], Awaitable[Tuple[Policy, Value]]]
...
class AsyncNode:
    ...
    def select_leaf(self, exploration_constant: float) -> "AsyncNode":
        """
        Selects a leaf node or terminal node from the tree according to the UCT value.
        """
    ...
    async def expand_children(self, evaluate_pos: AsyncEvaluatePos) -> None:
        """Expands the children of this leaf node and backpropagates the value up the tree."""
        logger = logging.getLogger(__name__)
...
async def mcts_async(
    pos: Pos, n_iterations: int, exploration_constant: float, eval_pos: AsyncEvaluatePos
) -> Policy:
    pass
    """
    Runs the MCTS algorithm to determine the best move from the given position.
